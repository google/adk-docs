...
- **`query_result_mode`**: Determines the format in which the `execute_sql` tool returns database query results. Setting this parameter to `QueryResultMode.DICT_LIST` configures the tool to return the results as a list of dictionaries. Import `QueryResultMode` from `google.adk.tools.spanner.settings`.
