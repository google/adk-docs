...
If you need a WebRTC or telephony bridge, ADK provides one. The [LiveKit runner](https://adk.dev/integrations/livekit/index.md) serves a live agent into a LiveKit room, handling media capture, playback, barge-in, and SIP, so there is no custom server to write.
