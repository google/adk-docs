...
3. To use tool with default credentials, install the [Google Cloud CLI]
   [installation guide](https://cloud.google.com/sdk/docs/install#installation_instructions) and run the following commands:
...
1.  Create a tool using `ApplicationIntegrationToolset` in the `tools.py` file:
  ...
  ApplicationIntegrationToolset also supports auth_scheme and auth_credential for dynamic OAuth2 authentication for Integration Connectors.       To use it, create a tool similar to this within the tools.py file:
...
2. Update the `agent.py` file and add the tool to your agent:
...
After completing the setup, go to [http://localhost:8000](http://localhost:8000), and choose
...
1. Create a tool using `ApplicationIntegrationToolset` in your `tools.py` file:
   ...
   > **Note:** You can provide a service account to be used instead of default credentials by generating a [Service Account Key](https://cloud.google.com/iam/docs/keys-create-delete#creating) and providing the correct Application Integration and Integration Connector IAM roles to the service account.
...
4. Configure \`\_\_init\_\_.py\` to expose your agent:

    ```py
    from . import agent
    ```

5. Start the Google ADK Web UI and use your agent:
