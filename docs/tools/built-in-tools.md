...
You can use multiple built-in tools together in the same agent, or in combination with your own custom tools. The recommended way to do this is by creating separate agents for each tool and then using `AgentTool` to combine them under a root agent.
