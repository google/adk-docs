...
information, see [Event Types & Payloads](#event-types).
...
-   **`event_allowlist`** (`Optional[List[str]]`, default: `None`): A list
    ...
    to the [Event Types & Payloads](#event-types) section.
-   **`event_denylist`** (`Optional[List[str]]`, default: `None`): A list of
    ...
    types, refer to the [Event Types & Payloads](#event-types) section.
...
```
### Event Types & Payloads {#event-types}
...
#### User & Generic Events (Event Stream)
