...
| `tool_thread_pool_config`       | `Optional[ThreadPoolConfig]`                 | `ThreadPoolConfig` (optional)         | N/A             | N/A                                                   | `None` / `undefined` / N/A / N/A                                                               | Configures a thread pool for running synchronous tools to avoid blocking the event loop. Critical for Live API mode.                                        |
