...
=== "Python - adk CLI"
    ...
    * `AGENT_PATH`: (Required) Positional argument specifying the path to the directory containing your agent's source code (for example, `$AGENT_PATH` in the examples, or `capital_agent/`). This directory must contain at least an `__init__.py` and your main agent file (for example, `agent.py`).
...
=== "Go - adkgo CLI"
    ...
    * `-p, --project_name`: Your Google Cloud project ID (for example, $GOOGLE_CLOUD_PROJECT).
