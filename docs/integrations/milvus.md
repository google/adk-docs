...
catalog_description: Vector search for agent memory and RAG context
...
The Milvus integration in
[`google-adk-community`](https://github.com/google/adk-python-community)
connects ADK Python agents to [Milvus](https://milvus.io/), an open-source
vector database. Use it for persistent semantic memory through
`MilvusMemoryService`, or expose a `milvus_similarity_search` retrieval tool for
RAG workflows through `MilvusToolset`.
...
- ADK for Python and `google-adk-community`
...
The `milvus` extra installs `pymilvus` and Milvus Lite. Milvus Lite 3.x local
storage is not compatible with older 2.x local database files or directories, so
create a new local database path for new projects.
...
- [ADK Python Community repository](https://github.com/google/adk-python-community)
