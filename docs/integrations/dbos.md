...
[DBOS](https://dbos.dev) is a durable execution framework for building reliable workflows and AI agents.
It integrates with ADK to make LLM calls, tool executions, and agent orchestration fault-tolerant and scalable.

With DBOS, agents resume exactly where they left off after crashes, deploys, or restarts. All backed by a database you own, no separate orchestration service required.
...
Add `DBOSPlugin` to your `Runner` and drive the agent from a `@DBOS.workflow()`.
Decorate tool functions with `@DBOS.step()` so their executions are checkpointed with configurable retries.
...
| --- | --- |
| Durable tool execution | Tool functions decorated with `@DBOS.step()` are checkpointed in the database with configurable retries on failure |
...
| Time-travel debugging | Replay any past workflow execution step-by-step. Fork and restart a workflow from a specific step for bug fixes |
