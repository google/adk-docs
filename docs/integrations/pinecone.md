...
=== "Go"
    ...
    === "Local MCP Server"
        ...
        func main() {
        	...
        	server := exec.CommandContext(ctx, "npx", "-y", "@pinecone-database/mcp")
        	// Forward only what npx needs, plus the Pinecone key. The parent environment
        	// may hold unrelated secrets, such as the GOOGLE_API_KEY read above.
        	server.Env = []string{"PINECONE_API_KEY=" + pineconeAPIKey}
        	...
        	pinecone, err := mcptoolset.New(mcptoolset.Config{
        		Transport: &mcp.CommandTransport{Command: server},
        	})
        	if err != nil {
        		log.Fatalf("Failed to create the Pinecone tool set: %v", err)
        	}
        	...
        	rootAgent, err := llmagent.New(llmagent.Config{
        		...
        		Name:        "pinecone_agent",
        		Instruction: "Help users manage and search their Pinecone vector indexes",
        		Toolsets:    []tool.Toolset{pinecone},
        	})
        	if err != nil {
