    ...
    === "Java"
        ...
        class MyAgent extends BaseAgent {
            ...
            protected Flowable<Event> runAsyncImpl(InvocationContext ctx) {
                // Direct access example
                String agentName = ctx.agent().name();
...
2.  **`ReadonlyContext`**
    ...
    *   **Key Contents:** `invocation_id`, `agent_name`, and a read-only *view* of the current `state`.
    ...
    === "Python"
        ...
        ```python
        # Instruction provider receiving ReadonlyContext
...
3.  **`CallbackContext`**
    ...
    *(Note: In Python and TypeScript, `CallbackContext` and `ToolContext` are unified into a single `Context` type.)*

    === "Python"

        ```python
        # Callback receiving Context
    ...
    === "Java"
        ...
        ```java
        // Callback receiving CallbackContext
...
4.  **`ToolContext`**
    ...
    *   **Key Capabilities (Adds to `CallbackContext`):**
    ...
    === "Python"
        ...
        ```python
        # Tool function receiving Context
        from google.adk.agents.context import Context
        from typing import Dict, Any

        # Assume this function is wrapped by a FunctionTool
        def search_external_api(query: str, context: Context) -> Dict[str, Any]:
            api_key = context.state.get("api_key")
            if not api_key:
                # Define required auth config
                # auth_config = AuthConfig(...)
                # context.request_credential(auth_config) # Request credentials automatically tracks actions
                return {"status": "Auth Required"}

            # Use the API key...
            print(f"Tool executing for query '{query}' using API key. Invocation: {context.invocation_id}")

            # Optionally search memory or list artifacts
            # relevant_docs = await context.search_memory(f"info related to {query}")
            # available_files = await context.list_artifacts()
    ...
    === "Java"
        ...
        ```java
        // Tool function receiving ToolContext
...
*   **Reading Session State:** Access data saved in previous steps or user/app-level settings. Use dictionary-like access on the `state` property.
    ...
    === "Python"
        ...
        def my_tool(context: Context, **kwargs):
            user_pref = context.state.get("user_display_preference", "default_mode")
            api_endpoint = context.state.get("app:api_endpoint") # Read app-level state
...
*   **Getting Current Identifiers:** Useful for logging or custom logic based on the current operation.
    ...
    === "Python"
        ...
        def log_tool_usage(context: Context, **kwargs):
            agent_name = context.agent.name
            inv_id = context.invocation_id
            func_call_id = getattr(context, 'function_call_id', 'N/A')
    ...
    === "Java"
        ...
        // In any context (ToolContext shown)
         ...
         public void logToolUsage(ToolContext toolContext) {
             String agentName = toolContext.agent().name();
...
*   **Accessing the Initial User Input:** Refer back to the message that started the current invocation.
    ...
    === "Java"
        ...
        public void checkInitialIntent(CallbackContext callbackContext) {
            ...
            if (callbackContext.userContent().isPresent() && !callbackContext.userContent().get().parts().isEmpty()) {
                initialText = callbackContext.userContent().get().parts().get(0).text();
...
*  **Passing Data Between Tools**
    ...
    === "Python"
        ...
        def get_user_profile(context: Context) -> dict:
            ...
            context.state["temp:current_user_id"] = user_id
            return {"profile_status": "ID generated"}

        # Tool 2 - Uses user ID from state
        def get_user_orders(context: Context) -> dict:
            user_id = context.state.get("temp:current_user_id")
            if not user_id:
...
*   **Updating User Preferences:**
    ...
    === "Python"
        ...
        def set_user_preference(context: Context, preference: str, value: str) -> dict:
...
*   **Document Summarizer Example Flow:**
        ...
        === "Java"
               ...
               public void saveDocumentReference(CallbackContext context, String filePath) {
                   ...
                   try{
                       ...
                       context.saveArtifact("document_to_summarize.txt", artifactPart).blockingAwait();
    ...
    2.  **Summarizer Tool:** Load the artifact to get the path/URI, read the actual document content using appropriate libraries, summarize, and return the result.
        ...
        === "Python"
            ...
            ```python
            # In the Summarizer tool function
            from google.adk.agents.context import Context
            from google.genai import types
            ...
            def summarize_document_tool(context: Context) -> dict:
                artifact_name = context.state.get("temp:doc_artifact_name")
                if not artifact_name:
                    return {"error": "Document artifact name not found in state."}

                try:
                    # 1. Load the artifact part containing the path/URI
                    artifact_part = context.load_artifact(artifact_name)
        ...
        === "Java"
            ...
            public Map<String, String> summarizeDocumentTool(ToolContext toolContext){
                ...
                try{
                    // 1. Load the artifact part containing the path/URI
                    Part artifactPart = toolContext.loadArtifact(artifactName).blockingGet();
...
*   **Listing Artifacts:** Discover what files are available.
    ...
    === "Python"
        ...
        def check_available_docs(tool_context: ToolContext) -> dict:
            try:
                artifact_keys = context.list_artifacts()
...
=== "Python"
    ...
    def call_secure_api(context: Context, request_data: str) -> dict:
        # 1. Check if credential already exists in state
        credential = context.state.get(AUTH_STATE_KEY)

        if not credential:
            # 2. If not, request it
            print("Credential not found, requesting...")
            try:
                context.request_credential(MY_API_AUTH_CONFIG)
        ...
        try:
            ...
            auth_credential_obj = context.get_auth_response(MY_API_AUTH_CONFIG)
            api_key = auth_credential_obj.api_key # Or access_token, etc.

            # Store it back in state for future calls within the session
            context.state[AUTH_STATE_KEY] = auth_credential_obj.model_dump() # Persist retrieved credential
            ...
            # context.state[AUTH_STATE_KEY] = None
...
=== "Python"
    ...
    ```python
    # Tool using memory search
    from google.adk.agents.context import Context

    def find_related_info(context: Context, topic: str) -> dict:
        try:
            search_results = context.search_memory(f"Information about {topic}")
...
=== "Python"
    ...
    ```python
    # Inside agent's _run_async_impl
    from google.adk.agents import BaseAgent
    from google.adk.agents.invocation_context import InvocationContext
    from google.adk.events import Event
    from typing import AsyncGenerator
