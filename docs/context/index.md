  ...
  In Python and TypeScript, `CallbackContext` and `ToolContext` have been replaced by the `Context` type.
    ...
    === "Python"
        ...
        def my_before_model_cb(callback_context: Context, llm_request: LlmRequest) -> Optional[LlmResponse]:
            ...
            callback_context.state["model_calls"] = call_count + 1 # Modify state (tracks delta)

            # Optionally load an artifact (artifact methods are async, so declare
            # the callback `async def`)
            # config_part = await callback_context.load_artifact("model_config.json")
...
- **Purpose:** Exposes the methods essential for tool execution, like handling authentication, searching memory, and listing artifacts. In Python these members are available on any context object, and the ones marked tool-only below raise `ValueError` unless a tool call is in progress.
- **Key Capabilities:**
    ...
    === "Python"
        ...
        def search_external_api(query: str, tool_context: ToolContext) -> Dict[str, Any]:
            ...
            # Optionally search memory or list artifacts (both are async, so declare
            # the tool function `async def`)
            ...
            if ctx.session.state.get("critical_error_flag"):
                ...
                yield Event(
                    ...
                    content=types.Content(
                        role="model",
                        parts=[types.Part.from_text(text="Stopping due to critical error.")],
                    ),
                )
...
Setting `ctx.end_invocation = True` gracefully stops the agent that sets it, and when that agent is the root agent, the whole request-response cycle stops. In Python, set the flag from an agent's `_run_async_impl` or `_run_live_impl`; callbacks and tools cannot set it. To stop an agent from a `before_agent_callback`, return content from the callback instead, and the framework sets `end_invocation` for you.
