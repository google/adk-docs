...
- **Key Capabilities (Adds to `ReadonlyContext`):**
    ...
    - **Artifact Methods:** `load_artifact(filename)` and `save_artifact(filename, part)` methods for interacting with the configured `artifact_service` (both `async` in Python).
