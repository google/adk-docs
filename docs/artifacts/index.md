...
1. **Handle Non-Textual Data:** Easily store and retrieve images, audio clips, video snippets, PDFs, spreadsheets, or any other file format relevant to your agent's function.  
2. **Persist Large Data:** Session state is generally not optimized for storing large amounts of data. Artifacts provide a dedicated mechanism for persisting larger blobs without cluttering the session state.  
...
4. **Share Outputs:** Enable tools or agents to generate binary outputs (like a PDF report or a generated image) that can be saved via `save_artifact` and later accessed by other parts of the application or even in subsequent sessions (if using user namespacing).  
5. **Cache Binary Data:** Store the results of computationally expensive operations that produce binary data (e.g., rendering a complex chart image) as artifacts to avoid regenerating them on subsequent requests.
...
* **Generate Reports/Files:**
    * A tool or agent generates a report (for instance, a PDF analysis, a CSV data export, an image chart).

* **Handle User Uploads:**

    * A user uploads a file (such as an image for analysis, a document for summarization) through a front-end interface.

* **Store Intermediate Binary Results:**

    * An agent performs a complex multi-step process where one step generates intermediate binary data (for example, audio synthesis, simulation results).

* **Persistent User Data:**

    * Store user-specific configuration or data that isn't a simple key-value state; remaining saved, accessible, and unchanged until it is manually deleted.

* **Cache Generated Binary Content:**

    * An agent frequently generates the same binary output based on certain inputs (like a company logo image or a standard audio greeting).
...
* **Interface:** Defined by the abstract base class `BaseArtifactService`. It is an abstract contract that determines how an agent manages an artifacts's lifecycle. Any concrete implementation must provide methods for:  
...
* **Configuration:** You provide an instance of an artifact service (for instance, `InMemoryArtifactService`, `GcsArtifactService`) when initializing the `Runner`. The `Runner` then makes this service available to agents and tools via the `InvocationContext`.
...
* **Best Practice:** Use descriptive names, potentially including file extensions (such as, `"monthly_report.pdf"`, `"user_avatar.jpg"`), although the extension itself doesn't dictate behavior – the `mime_type` does.
