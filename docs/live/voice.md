...
In web applications, transcription events need to be forwarded from the server to the browser and rendered in the UI. A common pattern is for the server to forward all ADK events (including transcription events) to the WebSocket client, and for the client to display transcriptions as speech bubbles with visual indicators for partial vs. finished transcriptions.
