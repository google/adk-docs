...
1. **ADK Detection**: ADK reads the schema definition to detect `meta.ui.resourceUri` and knows this tool supports an interactive UI.
1. **Client Display**: Upon tool execution, ADK signals the web UI (`adk web` or custom frontend) to fetch the UI resource and render an interactive widget instead of plain text.
...
- Return to the [Model Context Protocol Overview](https://adk.dev/tools-custom/mcp-tools/index.md) for basic setup.
- Explore [Custom Function Tools](https://adk.dev/tools-custom/function-tools/index.md) for in-process Python tools.
- Read the [ADK Deployment Guide](https://adk.dev/deploy/index.md) for full cloud configuration options.
