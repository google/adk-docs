...
GKE uses **container-based deployment** to a managed GKE Autopilot cluster. Your agent is packaged as a Docker container (same Dockerfile as Cloud Run), pushed to Artifact Registry, and deployed via Kubernetes manifests.
...
Manifests live in `deployment/k8s/`. Check these files for current configuration:

| File | Purpose |
|------|---------|
| **`deployment.yaml`** | Pod spec, container config, resource requests/limits, readiness/liveness probes, env vars, optional Cloud SQL proxy sidecar |
| **`service.yaml`** | LoadBalancer service exposing port 8080 |
| **`hpa.yaml`** | HorizontalPodAutoscaler (2-10 replicas, 70% CPU target) |
| **`pdb.yaml`** | PodDisruptionBudget (minAvailable: 1) |
| **`serviceaccount.yaml`** | Kubernetes ServiceAccount for Workload Identity |
...
GKE uses Workload Identity to map Kubernetes service accounts to GCP service accounts. The Kubernetes SA in `deployment/k8s/serviceaccount.yaml` is bound to `app_sa` via an `iam.workloadIdentityUser` IAM binding in Terraform.
...
Cloud SQL in GKE uses a **proxy sidecar container** in the pod (unlike Cloud Run which uses a Unix socket volume mount). The sidecar is configured in `deployment/k8s/deployment.yaml`.
...
- **Internal LoadBalancer** — Add the annotation `networking.gke.io/load-balancer-type: "Internal"` to `deployment/k8s/service.yaml` to restrict access to VPC-internal clients only.
